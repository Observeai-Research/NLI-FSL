...
def parse_argument():
	...
	parser.add_argument('--glove_path',type=str, default='../glove/glove.6B.50d.json')
...
def load_model(config, embedding):
	model = framework.HATT(config,embedding).to(config['device']) 
...
if __name__ == "__main__": 
	...
	for n_r in range(num_run): 
		...
		data = read_datasets(config["inter_data_path"], config['num_samples_per_class'], config['dataset'], n_r + 1, config['src'], config['tgt'], config['glove_path'])
