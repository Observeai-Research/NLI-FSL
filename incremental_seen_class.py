...
def helper(seen_labels, fsl_labels, baseclass, label_to_utt):
	train_data = {}
	train_data["label"] = []
	...
	for sl in seen_labels:
		...
		train_data["label"].extend(labels[:int((baseclass*size_train)/100)])
		train_data["utterance"].extend(imp_utts[:int((baseclass*size_train)/100)])
	...
	if inter_data_path[-1] == "/":
		os.makedirs(inter_data_path + str(baseclass) + "_percent/")
	else:
		os.makedirs(inter_data_path + "/" + str(baseclass) + "_percent/")

	train_data.to_csv(inter_data_path + str(baseclass) + "_percent/" + "train_seen.tsv", sep="\t", index=False, header=False)
	test_data_novel.to_csv(inter_data_path + str(baseclass) + "_percent/" + "test_novel.tsv", sep="\t", index=False, header=False)
	test_data_joint.to_csv(inter_data_path + str(baseclass) + "_percent/" + "test_joint.tsv", sep="\t", index=False, header=False)
	support1_novel.to_csv(inter_data_path + str(baseclass) + "_percent/" + "support_1_shots_novel", sep="\t", index=False, header=False)
	support1_seen.to_csv(inter_data_path + str(baseclass) + "_percent/" + "support_1_shots_seen", sep="\t", index=False, header=False)
	support1_joint.to_csv(inter_data_path + str(baseclass) + "_percent/" + "support_1_shots_joint", sep="\t", index=False, header=False)
	support5_novel.to_csv(inter_data_path + str(baseclass) + "_percent/" + "support_5_shots_novel", sep="\t", index=False, header=False)
	support5_seen.to_csv(inter_data_path + str(baseclass) + "_percent/" + "support_5_shots_seen", sep="\t", index=False, header=False)
	support5_joint.to_csv(inter_data_path + str(baseclass) + "_percent/" + "support_5_shots_joint", sep="\t", index=False, header=False)
...
def process_CLINC150(inter_data_path, num_shots, config):
	...
	for_fsl = [20,40,60,80,100]
	index = 0
	for fsl in for_fsl:
		index = index + 1
		helper(seen_labels, fsl_labels, fsl, label_to_utt)
	return
